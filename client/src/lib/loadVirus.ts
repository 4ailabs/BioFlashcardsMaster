...
import virusJson from './virus.json';
...
export function loadVirusFromJson(): { virusAdn: Flashcard[], virusArn: Flashcard[] } {
  try {
    console.log(`Cargando ${virusJson.length} virus desde JSON`);
    ...
    virusJson.forEach((virus: any) => {
      ...
      // Construir las notas
      let notes = '';
      if (virus.notas?.enfermedades_relacionadas) {
        notes += `Enfermedades relacionadas: ${virus.notas.enfermedades_relacionadas}\n\n`;
      }
      ...
      // Agregar a la lista correspondiente según la categoría
      if (category === 'virus_adn') {
